...
c[f]}return g}function y(){var c=arguments[0]||{},g=1,f=arguments.length,i=false,k;if(typeof c==="boolean"){i=c;c=arguments[1]||{};g=2}if(typeof c!=="object"&&!V(c))c={};for(;g<f;g++)if((k=arguments[g])!=null)for(var n in k){var p=c[n],q=k[n];if(c!==q)if(i&&q&&typeof q==="object"&&!q.nodeType)c[n]=y(i,p||(q.length!=null?[]:{}),q);else if(q!==aa)c[n]=q}return c}function L(c,g,f){var i,k=0,n=c.length,p=n===aa||V(c);if(f)if(p)for(i in c){if(g.apply(c[i],f)===false)break}else for(;k<n;){if(g.apply(c[k++],
f)===false)break}else if(p)for(i in c){if(g.call(c[i],i,c[i])===false)break}else for(f=c[0];k<n&&g.call(f,k,f)!==false;f=c[++k]);return c}function ha(c,g,f){for(var i=[],k=0,n=c.length;k<n;k++)!f!==!g(c[k],k)&&i.push(c[k]);return i}function F(c,g){for(var f=[],i,k=0,n=c.length;k<n;k++){i=g(c[k],k);if(i!=null)f[f.length]=i}return f.concat.apply([],f)}function z(c){var g=[];if(typeof c=="object"){for(var f in c)g[g.length]=encodeURIComponent(f)+"="+encodeURIComponent(c[f]);return g.join("&").replace(ia,
"+")}return c}function C(c){c=y(true,c,y(true,{},pa,c));var g,f,i=c.context||w,k=c.type.toUpperCase();if(c.data&&c.processData&&typeof c.data!=="string")c.data=z(c.data);if(c.cache===false&&k==="GET"){var n=oa(),p=c.url.replace(ea,"$1_="+n+"$2");c.url=p+(p===c.url?(K.test(c.url)?"&":"?")+"_="+n:"")}if(c.data&&k==="GET")c.url+=(K.test(c.url)?"&":"?")+c.data;var q=false,r=c.xhr();c.username?r.open(k,c.url,c.async,c.username,c.password):r.open(k,c.url,c.async);try{c.data&&r.setRequestHeader("Content-Type",
c.contentType);if(c.ifModified){la[c.url]&&r.setRequestHeader("If-Modified-Since",la[c.url]);Z[c.url]&&r.setRequestHeader("If-None-Match",Z[c.url])}r.setRequestHeader("X-Requested-With","XMLHttpRequest");r.setRequestHeader("Accept",c.dataType&&c.accepts[c.dataType]?c.accepts[c.dataType]+", */*":c.accepts._default)}catch(W){}if(c.beforeSend&&c.beforeSend.call(i,r,c)===false){r.abort();return false}var B=function(ca){if(!r||r.readyState===0){if(O){clearInterval(O);O=null}}else if(!q&&r&&(r.readyState===
4||ca==="timeout")){q=true;if(O){clearInterval(O);O=null}var A;if(ca==="timeout")A="timeout";else{a:{try{A=!r.status&&location.protocol==="file:"||r.status>=200&&r.status<300||r.status===304||r.status===1223||r.status===0;break a}catch(ta){}A=false}if(A){if(A=c.ifModified){A=r;var S=c.url,G=A.getResponseHeader("Last-Modified"),M=A.getResponseHeader("Etag");if(G)la[S]=G;if(M)Z[S]=M;A=A.status===304||A.status===0}A=A?"notmodified":"success"}else A="error";A=A}g=A;if(g==="success")try{A=r;var ma=c.dataType;
S=c;var qa=A.getResponseHeader("content-type"),ra=ma==="xml"||!ma&&qa&&qa.indexOf("xml")>=0,a=ra?A.responseXML:A.responseText;if(ra&&a.documentElement.nodeName==="parsererror")throw"parsererror";if(S&&S.dataFilter)a=S.dataFilter(a,ma);if(typeof a==="string")if(ma==="json")a=typeof P==="object"&&P.parse?P.parse(a):(new Function("return "+a))();f=a}catch(b){g="parsererror"}if(g==="success"||g==="notmodified")c.success&&c.success.call(i,f,g);else if(c.error)c.error.call(c.context||w,r,g,void 0);c.complete&&
c.complete.call(i,r,g);ca&&r.abort();if(c.async)r=null}};if(c.async){var O=setInterval(B,13);c.timeout>0&&setTimeout(function(){r&&!q&&B("timeout")},c.timeout)}try{r.send(k==="POST"||k==="PUT"?c.data:null)}catch(T){if(c.error)c.error.call(c.context||w,r,null,T)}c.async||B();return r}function s(){}function Q(c){function g(){q=true;w[k]=aa;try{delete w[k]}catch(r){}p.onload=p.onreadystatechange=null;n&&p.parentNode&&n.removeChild(p)}c=y({},c);var f=""+z(c.data),i=c.context||w,k="jsonp"+H++,n=v.getElementsByTagName("head")[0]||
v.documentElement,p=v.createElement("script");f=(f?f+"&":"")+(c.jsonp||"callback")+"="+k;c.url+=(K.test(c.url)?"&":"?")+f;p.src=c.url;if(c.scriptCharset)p.charset=c.scriptCharset;var q=false;w[k]=function(r){c.success&&c.success.call(i,r,"success");g()};p.onload=p.onreadystatechange=function(){if(!q&&(!this.readyState||this.readyState==="loaded"||this.readyState==="complete")){c.error&&c.error.call(i,"error");g()}};c.timeout>0&&setTimeout(function(){if(!q){c.error&&c.error.call(i,"timeout");g();w[k]=
s}},c.timeout);n.insertBefore(p,n.firstChild);return aa}function ja(c,g){this._setting();this.options={jsonp:false,server:null,ticket:null,domain:null,timeout:4E4,url:{send:null}};y(this.options,g)}function na(c,g,f){if(typeof g!="undefined"){f=f||{};if(g===null){g="";f=y({},f);f.expires=-1}var i="";if(f.expires&&(typeof f.expires=="number"||f.expires.toUTCString)){if(typeof f.expires=="number"){i=new Date;i.setTime(i.getTime()+f.expires*24*60*60*1E3)}else i=f.expires;i="; expires="+i.toUTCString()}var k=
f.path?"; path="+f.path:"",n=f.domain?"; domain="+f.domain:"";f=f.secure?"; secure":"";v.cookie=[c,"=",encodeURIComponent(g),i,k,n,f].join("")}else{g=null;if(v.cookie&&v.cookie!=""){f=v.cookie.split(";");for(i=0;i<f.length;i++){k=Y(f[i]);if(k.substring(0,c.length+1)==c+"="){g=decodeURIComponent(k.substring(c.length+1));break}}}return g}}function da(c,g){if($){var f=new Date,i=["[",f.getHours(),":",f.getMinutes(),":",f.getSeconds(),"-",f.getMilliseconds(),"]"].join("");f=i+g+P.encode(c);w.console&&
w.console.log(i,g,c);i=v.getElementById("webim-log")||v.body;w.air&&w.air.trace(f);if(i){var k=v.createElement("P");k.innerHTML=f;i.appendChild(k)}}}function U(c,g){this.options=y({},U.defaults,g);this._init(c,g)}function R(c){return c&&c.split?c.split(","):ba(c)?c:parseInt(c)?[parseInt(c)]:[]}function t(c,g,f){function i(k,n){this.data=k;this.options=y({},i.defaults,n);V(this._init)&&this._init()}i.defaults=g;y(i.prototype,fa,f);U[c]=i}var x=Object.prototype.toString,fa={option:function(c,g){var f=
...
this.setting=new U.setting;this.buddy=new U.buddy(null,{loadDelay:!this.status.get("b")});this.room=new U.room;this.history=new U.history;this.connection=new ja(null,{jsonp:true});this._initEvents()},_ready:function(c){var g=this;g._unloadFun=w.onbeforeunload;w.onbeforeunload=function(){g._refresh()};g.trigger("ready",[c])},_go:function(){var c=this.data,g=this.history,f=this.buddy,i=this.room;g.option("userInfo",c.user);f.handle(c.buddies);L(c.buddies,function(n,p){g.init("unicast",p.id,p.history)});
f.online(c.online_buddy_ids);L(c.buddies,function(n,p){g.init("multicast",p.id,p.history)});f=this.setting.get("blocked_rooms");var k=c.rooms;ba(f)&&k&&L(f,function(n,p){k[p]&&(k[p].blocked=true)});i.handle(k);i.options.ticket=c.connection.ticket;if((i=c.new_messages)&&i.length){L(i,function(n,p){p["new"]=true});this.trigger("message",[i])}this.trigger("go",[c]);this.connection.connect(c.connection)},_stop:function(c){w.onbeforeunload=this._unloadFun;this.data.user.presence="offline";this.data.user.show=
"unavailable";this.buddy.clear();this.trigger("stop",c)},autoOnline:function(){return!this.status.get("o")},_initEvents:function(){function c(p){return p.type=="offline"}function g(p){return p.type=="online"}function f(p){return p.from}var i=this,k=i.history,n=i.buddy;i.connection.bind("connect",function(){}).bind("data",function(p){i.handle(p)}).bind("error",function(){i._stop("connect error")}).bind("close",function(){i._stop("disconnect")});i.bind("message",function(p){for(var q=[],r=p.length,
W=i.data.user.id,B,O,T,ca=0;ca<r;ca++){B=p[ca];T=B.type;O=T=="unicast"?B.to==W?B.from:B.to:B.to;B.id=O;T=="unicast"&&!B["new"]&&q.push(O)}q.length&&n.online(q);k.handle(p)});i.bind("presence",function(p){offline=ha(p,c);online=ha(p,g);n.online(F(online,f));n.offline(F(offline,f))})},handle:function(c){c.messages&&c.messages.length&&this.trigger("message",[c.messages]);c.presences&&c.presences.length&&this.trigger("presence",[c.presences]);c.statuses&&c.statuses.length&&this.trigger("status",[c.statuses])},
sendMsg:function(c){c.ticket=this.data.connection.ticket;C({type:"post",url:this.options.urls.message,cache:false,data:c})},sendStatus:function(c){c.ticket=this.data.connection.ticket;C({type:"post",url:this.options.urls.status,cache:false,data:c})},sendPresence:function(c){c.ticket=this.data.connection.ticket;this.status.set("s",c.show);C({type:"post",url:this.options.urls.presence,cache:false,data:c})},online:function(c){var g=this,f=g.status,i=[],k=[],n=f.get("tabs"),p=f.get("tabIds");p&&p.length&&
n&&L(n,function(q){q[0]=="b"&&i.push(q.slice(2));q[0]=="r"&&k.push(q.slice(2))});c=y({buddy_ids:i.join(","),room_ids:k.join(","),show:f.get("s")||"available"},c);g._ready(c);f.set("o",false);f.set("s",c.show);C({type:"post",dataType:"json",data:c,url:g.options.urls.online,success:function(q){if(!q||!q.user||!q.connection)g._stop("online error");else{q.user=y(g.data.user,q.user,{presence:"online"});g.data=q;g._go()}},error:function(){g._stop("online error")}})},offline:function(){var c=this.data;this.status.set("o",
true);this.connection.close();this._stop("offline");C({type:"post",url:this.options.urls.offline,type:"post",cache:false,data:{status:"offline",ticket:c.connection.ticket}})},_refresh:function(){var c=this.data;!c||!c.connection||!c.connection.ticket||C({type:"post",url:this.options.urls.refresh,type:"post",cache:false,data:{ticket:c.connection.ticket}})}});w.webim=U;y(U,{version:"1.0.0pre",defaults:{urls:{online:"webim/online",offline:"webim/offline",message:"webim/message",presence:"webim/presence",
...
var i=c.length,k,n,p=this.options.userInfo.id;if(i){for(var q=0;q<i;q++){k=c[q];r=k.type;if((n=r=="unicast"?k.to==p?k.from:k.to:k.to)&&r){f[r][n]=f[r][n]||[];f[r][n].push(k)}}for(var r in f)for(n in f[r]){k=f[r][n];if(g[r][n]){g[r][n]=g[r][n].concat(k);this._triggerMsg(r,n,k)}else this.load(r,n)}}},_triggerMsg:function(c,g,f){this.trigger(c,[g,f])},clear:function(c,g){var f=this.options;this.data[c][g]=[];this.trigger("clear",[c,g]);C({url:f.urls.clear,cache:false,data:{type:c,id:g}})},init:function(c,
...
H(G(this.options.template))){this.options.className&&D(this.element,this.options.className);this.$=fa(this.element)}Z(this._init)&&this._init();Z(this._initEvents)&&this._initEvents()}e.defaults=b;f(e.prototype,q,K.prototype,d);x[a]=e}function ea(a,b){x.apps[a]=b||{}}function ia(a,b){return b?a=="b"||a=="buddy"||a=="unicast"?"b_"+b:"r_"+b:a}function pa(){v.selection&&(this.caretPos=v.selection.createRange())}var la=webim.idsArray,Z=webim.isFunction,P=webim.isArray,$=webim.isObject,c=webim.trim,g=
webim.makeArray,f=webim.extend,i=webim.each,k=webim.grep,n=webim.ajax,p=webim.model,q=webim.objectExtend,r=webim.map,W=false,B=[],O=false;R.timeSkew=0;R.init=function(a){R.timeSkew=(new Date).getTime()-parseFloat(a)};f(R.prototype,{getTime:function(){var a=this.date,b=a.getHours();a=a.getMinutes();if(a<10)a="0"+a;return b+":"+a+""},getDay:function(a){var b=this.date;if(a){a=new Date;a.setHours(0);a.setMinutes(0);a.setSeconds(0);a.setMilliseconds(0);a=a.getTime()-b.getTime();if(a<=0)return t("dt:today");
...
d:"/sound/sound.mp3")}catch(e){}}}}(),ca=function(){var a=false;s(w,"focus",function(){a=false});s(w,"blur",function(){a=true});var b=v.title,d=0,e=false;return function(h,j){if(a){if(l){clearInterval(l);d=0;e=false}var l=setInterval(function(){d++;e=!e;if(d==j||!a){clearInterval(l);d=0;e=false}v.title=e?"["+h+"]"+b:b},1500)}}}(),A={},ta=function(a,b){return A[b]||""};t.locale="zh-CN";t.dictionary={};t.store=function(a,b){var d=t.dictionary;$(d[a])||(d[a]={});f(d[a],b)};f(x.prototype,q,{render:function(){var a=
...
function(){a._updateStatus()});b.bind("message",function(l){for(var m=false,o=l.length,u,I=b.data.user.id,N,X,ga=0;ga<o;ga++){u=l[ga];N=u.id;type=u.type;(X=j.chat(type,N))&&X.status("");if(!X){u.type==="unicast"?a.addChat(type,N,null,null,u.nick):a.addChat(type,N);X=j.chat(type,N)}X&&h.get("msg_auto_pop")&&!j.activeTabId&&j.focusChat(N);X.window.notifyUser("information","+1");N=X.window.pos;N==-1&&j.setNextMsgNum("+1");N==1&&j.setPrevMsgNum("+1");if(u.from!=I)m=true}if(m){T.play("msg");ca(t("new message"),
5)}});b.bind("status",function(l){i(l,function(m,o){var u=b.data.user.id,I=o.from;if(!(u!=o.to&&u!=o.from))(u=j.chat("buddy",I))&&u.status(o.show)})});e.bind("unicast",function(l,m){var o=j.chat("unicast",l);o&&o.history.add(m)});e.bind("multicast",function(l,m){var o=j.chat("multicast",l);o&&o.history.add(m)});e.bind("clear",function(l,m){var o=j.chat(l,m);o&&o.history.clear()})},__status:false,_initStatus:function(){var a=this,b=a.layout;if(a.__status)return b.updateAllChat();a.__status=true;var d=
a.im.status,e=d.get("tabs"),h=d.get("tabIds"),j=d.get("p");d=d.get("a");h&&h.length&&e&&i(e,function(l,m){var o=l.slice(2);a.addChat(l[0],o,{},{isMinimize:true});b.chat(l).window.notifyUser("information",m.n)});j&&(b.prevCount=j)&&b._fitUI();d&&b.focusChat(d)},addChat:function(a,b,d,e,h){a=a=="b"||a=="buddy"||a=="unicast"?"buddy":"room";var j=this,l=j.layout,m=j.im,o=j.im.history,u=m.buddy,I=m.room;if(!l.chat(a,b))if(a=="room"){var N=o.multicast(b),X=I.get(b);h=X||{id:b,nick:h||b};h.presence="online";
l.addChat(a,h,f({history:N,block:true,emot:true,clearHistory:false,member:true,msgType:"multicast"},d),e);N||o.load("multicast",b);var ga=l.chat(a,b);ga.bind("sendMsg",function(J){m.sendMsg(J);o.handle(J)}).bind("select",function(J){u.online(J.id);j.addChat("buddy",J.id,null,null,J.nick);l.focusChat("buddy",J.id)}).bind("block",function(J){I.block(J.id)}).bind("unblock",function(J){I.unblock(J.id)}).window.bind("close",function(){ga.options.info.blocked&&I.leave(b)});setTimeout(function(){ga.options.info.blocked?
I.join(b):I.initMember(b)},500);P(h.members)&&i(h.members,function(J,sa){ga.addMember(sa.id,sa.nick,sa.id==m.data.user.id)})}else{N=o.unicast(b);h=(X=u.get(b))||{id:b,nick:h||b};l.addChat(a,h,f({history:N,block:false,emot:true,clearHistory:true,member:false,msgType:"unicast"},d),e);X||u.update(b);N||o.load("unicast",b);l.chat(a,b).bind("sendMsg",function(J){m.sendMsg(J);o.handle(J)}).bind("sendStatus",function(J){m.sendStatus(J)}).bind("clearHistory",function(J){o.clear("unicast",J.id)})}},_updateStatus:function(){var a=
...
b.prevCount--}var h=b.$.tabs,j=parseFloat(h.style.left);d=-1*b.tabWidth*b.nextCount;var l=parseInt(500/13),m=1,o=(d-j)/l,u=setInterval(function(){h.style.left=j+o*m+"px";if(m==l){if(b.slideing)b._slide(a);else{b._fitUI();b._slideReset()}clearInterval(u)}else m++},13)}},_slideUp:function(){this.slideing=false},_slideSetup:function(a){var b=this.$,d=b.tabsWrap;b=b.tabs;if(!this._tabsWidth)this._tabsWidth=b.clientWidth;if(a)this._tabsWidth=null;d.style.position=a?"":"relative";d.style.overflow=a?"visible":
...
_renderMsg:function(a){a=f({},a);M.call(this,"render",[null,this.ui({msg:a})]);var b=a.from,d=a.to,e=a.timestamp,h=a.body,j=true,l=this._lastLogItem,m=[],o;o=a.nick;if(l&&l.to==d&&l.from==b&&e-l.timestamp<6E4)j=false;if(j){this._lastLogItem=a;a=new R(e);m.push('<h4 class="ui-state-default"><span class="webim-gray">');m.push(a.getDay(true));m.push(" ");m.push(a.getTime());m.push("</span>");m.push(o);m.push("</h4>")}m.push("<p>");m.push(h);m.push("</p>");return m.join("")},_renderDateBreak:function(a){var b=
...
_sendMsg:function(a){var b=this.options,d=b.info;a={type:b.msgType,to:d.id,from:b.user.id,offline:d.presence!="online",timestamp:(new Date).getTime(),body:a};M.call(this,"send",[null,this.ui({msg:a})]);this.trigger("sendMsg",a)},_inputkeypress:function(a){if(a.keyCode==13)if(a.ctrlKey){this.insert("\n",true);return true}else{var b=ja(a),d=b.value;if(c(d)){this._sendMsg(d);b.value="";Q(a)}}else this._typing()},_onFocusInput:function(a){var b=this;ja(a);(w.getSelection?w.getSelection().toString():v.selection?
v.selection.createRange().text:"")||w.setTimeout(function(){b.$.input.focus()},0)},_initEvents:function(){var a=this,b=a.$,d=t("input notice"),e=b.input;a.history.bind("update",function(){a._adjustContent()}).bind("clear",function(){a.notice(t("clear history notice"),3E3)});s(e,"keyup",function(){pa.call(this)});s(e,"click",pa);s(e,"select",pa);s(e,"focus",function(){E(this,"webim-gray");if(this.value==d)this.value=""});s(e,"blur",function(){if(this.value==""){D(this,"webim-gray");this.value=d}});
s(e,"keypress",function(h){a._inputkeypress(h)});s(b.content,"click",function(h){a._onFocusInput(h)})},_updateInfo:function(a){var b=this.$;b.userPic.setAttribute("href",a.url);b.userPic.firstChild.setAttribute("defaultsrc",a.default_pic_url?a.default_pic_url:"");setTimeout(function(){b.userPic.firstChild.setAttribute("src",a.pic_url)},100);b.userStatus.innerHTML=a.status||"&nbsp";this.window.title(a.nick,a.show)},insert:function(a,b){var d=this.$.input;d.focus();if(b){a||(a="");if(d.setSelectionRange){var e=
...
[{show:a}])},destroy:function(){}});ea("buddy",{init:function(a){var b=this,d=b.im,e=d.buddy,h=b.layout,j=b.buddy=new x.buddy(null,a);h.addWidget(j,{title:t("buddy"),icon:"buddy",sticky:d.setting.get("buddy_sticky"),className:"webim-buddy-window",isMinimize:!d.status.get("b"),titleVisibleLength:19});d.setting.bind("update",function(m,o){m=="buddy_sticky"&&j.window.option("sticky",o)});j.bind("select",function(m){b.addChat("buddy",m.id);b.layout.focusChat("buddy",m.id)}).bind("online",function(){d.online()});
j.window.bind("displayStateChange",function(m){if(m!="minimize"){e.option("loadDelay",false);d.status.set("b",1);e.loadDelay()}else{d.status.set("b",0);e.option("loadDelay",true)}});e.bind("online",function(m){j.add(m);j.notice("count",e.count({presence:"online"}))});e.bind("onlineDelay",function(){j.notice("count",e.count({presence:"online"}))});var l=function(m){return $(m)?m.id:m};e.bind("offline",function(m){j.remove(r(m,l));j.notice("count",e.count({presence:"online"}))});e.bind("update",function(m){j.update(m)});
...
"("+h.count+")"}C(d);delete e[b]}}},select:function(a){(a=this.li[a])&&a.firstChild.click();return a},destroy:function(){}});ea("room",{init:function(){function a(o){var u=o.nick;o=f({},o,{group:"group",nick:u+"("+(parseInt(o.count)+"/"+parseInt(o.all_count))+")"});l.updateChat(o);o.blocked&&(o.nick=u+"("+t("blocked")+")");m.li[o.id]?m.update(o):m.add(o)}var b=this,d=b.im,e=d.room,h=d.setting,j=d.data.user,l=b.layout,m=b.room=(new webim.ui.room(null)).bind("select",function(o){b.addChat("room",o.id);
b.layout.focusChat("room",o.id)});l.addWidget(m,{title:t("room"),icon:"room",sticky:d.setting.get("buddy_sticky"),onlyIcon:true,isMinimize:true},"notification");d.setting.bind("update",function(o,u){o=="buddy_sticky"&&m.window.option("sticky",u)});e.bind("join",function(o){a(o)}).bind("leave",function(){}).bind("block",function(o,u){h.set("blocked_rooms",u);a(e.get(o));e.leave(o,j)}).bind("unblock",function(o,u){h.set("blocked_rooms",u);a(e.get(o));e.join(o,j)}).bind("addMember",function(o,u){var I=
l.chat("room",o);I&&I.addMember(u.id,u.nick,u.id==d.data.user.id);a(e.get(o))}).bind("removeMember",function(o,u){var I=l.chat("room",o);I&&I.removeMember(u.id,u.nick);a(e.get(o))})},ready:function(){},go:function(){},stop:function(){}});K("room",{template:'<div id="webim-room" class="webim-room">\t\t<div id=":search" class="webim-room-search ui-state-default ui-corner-all"><em class="ui-icon ui-icon-search"></em><input id=":searchInput" type="text" value="" /></div>\t\t\t<div class="webim-room-content">\t\t\t\t<div id=":empty" class="webim-room-empty"><%=empty room%></div>\t\t\t\t\t<ul id=":ul"></ul>\t\t\t\t\t\t</div>\t\t\t\t\t\t\t</div>',
...
300)a.style.height="300px"},add:function(a){var b=this;if(P(a))i(a,function(e,h){b.add(h)});else{var d=b.$;z(d.empty);d.ul.appendChild(H(b._li_tpl(a)))}},destroy:function(){}});p("notification",{url:"webim/notifications"},{grep:function(a){return a&&a.text},handle:function(a){a=k(g(a),this.grep);a.length&&this.trigger("data",[a])},load:function(){n({url:this.options.url,cache:false,dataType:"json",context:this,success:this.handle})}});ea("notification",{init:function(a){var b=this.im,d=this.layout,
